...
SELECT
  ...
  strasse,
  CASE
    WHEN COALESCE(strasse, '') = '' THEN NULL
    ELSE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(strasse), ' ', ''), '/', ''), '-', ''), '\', ''), ')', ''), '(', ''), '.', ''), 'straße', 'str'), 'strasse', 'str')
  END,
  plz,
  ort,
  telefon,
  ...
  (SELECT COUNT(*) FROM rechnung WHERE kunde_id = kunde.id AND vorlage = 0),
  (SELECT COUNT(*) FROM bestellung WHERE kunde_id = kunde.id),
  (SELECT COUNT(*) FROM angebot WHERE kunde_id = kunde.id),
  (SELECT COUNT(*) FROM rechnung WHERE kunde_id = kunde.id AND vorlage = 1)
FROM
  kunde;
